...
export default {
  ...
  methods: {
    ...
    async initSave () {
      ...
      const res = await toUpdateBatchMenu(arr)
      this.inputArr3 = res.records
      this.getData()
      this.$nextTick(() => {
        this.initBol = true
      })
    ...
    async saveEditData () {
      if (this.idArr2.length > 0) {
        await toRemoveBatchMenu({ menus: this.idArr2.map(obj => ({ id: obj.id })) })
        this.idArr2 = []
        this.getData()
      }
      // 顶级角色
      ...
      menus = [...menus, ...arr]
      const res = await toUpdateBatchMenu(menus)
      if (res.code === 200) {
        this.itemInfo2.children = res.records
        this.inputArr2 = []
        this.idArr2 = []
      }
      this.$message.success(res.msg)
    ...
    async save () {
      if (this.idArr.length > 0) {
        await toRemoveBatchMenu({ menus: this.idArr.map(obj => ({ id: obj })) })
      }
      if (this.parentId === 0) {
        console.log('执行了1')
        ...
        if (menus.length > 0) {
          const res = await toUpdateBatchMenu(menus)
          if (res.code === 200) {
            this.itemInfo.children = res.records
            this.inputArr = []
            this.idArr = []
          }
        ...
        console.log('执行了2')
        let menus =
          this.itemInfo && this.itemInfo.children
            ? this.itemInfo.children.map(item => {
              return {
                id: item.id,
                parentId: item.parentId,
                menuText: item.menuText,
